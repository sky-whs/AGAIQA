...
def train_epoch(epoch, net, criterion, optimizer, scheduler, train_loader, criterion1):
    losses = []
    net.train()
    ...
    for index,data in enumerate(tqdm(train_loader)):
        ...
        labels1 = labels.unsqueeze(1).unsqueeze(1).unsqueeze(1)
        pred_d, pred_d1 = net(x_d)

        optimizer.zero_grad()
        loss1 = criterion1(pred_d1,labels1)

        print(loss1, 'DO loss')
        loss = criterion(torch.squeeze(pred_d), labels) + loss1 * 0.005
...
def eval_epoch(config, epoch, net, criterion, test_loader):
    with torch.no_grad():
        ...
        for data in tqdm(test_loader):
            ...
            pred_batch_numpy1 = pred_or.data.cpu().numpy()
            pred_epoch1 = np.append(pred_epoch1, pred_batch_numpy1)
        
        # compute correlation coefficient
        rho_s, _ = spearmanr(np.squeeze(pred_epoch), np.squeeze(labels_epoch))
        rho_p, _ = pearsonr(np.squeeze(pred_epoch), np.squeeze(labels_epoch))

        rho_s1, _ = spearmanr(np.squeeze(pred_epoch1), np.squeeze(labels_epoch))
        rho_p1, _ = pearsonr(np.squeeze(pred_epoch1), np.squeeze(labels_epoch))
...
if __name__ == '__main__':
    ...
    # model defination
    net = AGAIQA(embed_dim=config.embed_dim, num_outputs=config.num_outputs, dim_mlp=config.dim_mlp,
    ...
    for epoch in range(0, config.n_epoch):
        ...
        loss_val, rho_s, rho_p = train_epoch(epoch, net, criterion, optimizer, scheduler, train_loader, criterion1)
